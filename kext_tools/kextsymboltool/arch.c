...
NXGetLocalArchInfo(void)
{
     ...
     */
    if (strcmp(uts.machine, "i386") == 0) {
	hbi.cpu_type = CPU_TYPE_I386;
	hbi.cpu_subtype = CPU_SUBTYPE_I386_ALL;
    } else if (strcmp(uts.machine, "i686") == 0) {
        hbi.cpu_type = CPU_TYPE_I386;
        hbi.cpu_subtype = CPU_SUBTYPE_I386_ALL;
    } else if (strcmp(uts.machine, "x86_64") == 0) {
	hbi.cpu_type = CPU_TYPE_X86_64;
	hbi.cpu_subtype = CPU_SUBTYPE_X86_64_ALL;
    } else if (strcmp(uts.machine, "armv6l") == 0) {
	hbi.cpu_type = CPU_TYPE_ARM;
	hbi.cpu_subtype = CPU_SUBTYPE_ARM_V6;
    } else if (strcmp(uts.machine, "armv7l") == 0) {
	hbi.cpu_type = CPU_TYPE_ARM;
	hbi.cpu_subtype = CPU_SUBTYPE_ARM_V7;
    } else
        return(NULL);
